    ...
    <div class="module">
      <module-title :indicator-active="dialsAreWithinMargin" :module-color="moduleColor">
        ...
        <module-display
          ...
          module="lfo"
          fill="#5bd484"
          :knobs="[{name: 'Rate', min:1, max:100, value: this.frequencyOpponent},
                   {name: 'Amount', min:0, max:4000, value: this.amountOpponent},
                   {name: 'Shape', min:0, max:3, value: this.typeOpponent},
      ...
      >
        <module-button color="#5bd484" shape="sine" :isPressed="typeOpponent==='sine'" />
        <module-button color="#5bd484" shape="square" :isPressed="typeOpponent==='square'" />
        <module-button color="#5bd484" shape="sawtooth" :isPressed="typeOpponent==='sawtooth'"/>
        <module-button color="#5bd484" shape="triangle" :isPressed="typeOpponent==='triangle'" />
        <p>MOVEMENT</p>
      </div>
        </div>
    </div>
...
export default {
  ...
  computed: {
    ...
    ...mapState({
      ...
      frequencyOpponent: state => state.gameState.opponent.lfo.frequency,
      typeOpponent: state => state.gameState.opponent.lfo.type,
      amountOpponent: state => state.gameState.opponent.lfo.amount,
