<template>
  <div class="module">
    ...
    <module-display
      fill="#6e01d1"
      module="filter"
      :knobs="[{name: 'type', min: 0, max: 2, value: typeOpponent},
                {name: 'frequency', min: 0, max: 100, value: cutOffFreqOpponent},
                {name: 'q', min: 0, max: 100, value: setQOpponent},
    ...
    <div class="knobs">
      ...
      >
        <module-button color="#6e01d1" shape="lowpass" :isPressed="typeOpponent==='lowpass'" />
        <module-button color="#6e01d1" shape="highpass" :isPressed="typeOpponent==='highpass'" />
        <module-button color="#6e01d1" shape="bandpass" :isPressed="typeOpponent==='bandpass'" />
        <p>SHAPE</p>
      </div>
    </div>
  </div>
...
export default {
  ...
  computed: {
    ...
    ...mapState({
      ...
      cutOffFreqOpponent: state => state.gameState.opponent.filter.cutOffFreq,
      typeOpponent: state => state.gameState.opponent.filter.type,
      setQOpponent: state => state.gameState.opponent.filter.setQ,
